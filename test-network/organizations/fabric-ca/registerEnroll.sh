...
function createPengepul() {
  ...
  infoln "Generate the user msp"
  set -x
  fabric-ca-client enroll -u https://user1:user1pw@localhost:8054 --caname ca-petani -M ${PWD}/organizations/peerOrganizations/pengepul.coedinus.com/users/User1@pengepul.coedinus.com/msp --tls.certfiles ${PWD}/organizations/fabric-ca/pengepul/tls-cert.pem
  { set +x; } 2>/dev/null
