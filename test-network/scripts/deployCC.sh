...
approveForMyOrg() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.coedinus.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.coedinus.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} $PEER_CONN_PARMS --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
chaincodeInvokeInit() {
  ...
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.coedinus.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS --isInit -c ${fcn_call} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
## Install chaincode on peer0.petani and peer0.pengepul
infoln "Installing chaincode on peer0.petani..."
installChaincode 1
infoln "Install chaincode on peer0.pengepul..."
installChaincode 2

## query whether the chaincode is installed
queryInstalled 1

## approve the definition for petani
approveForMyOrg 1

## check whether the chaincode definition is ready to be committed
## expect petani to have approved and pengepul not to
checkCommitReadiness 1 "\"petaniMSP\": true" "\"pengepulMSP\": false"
checkCommitReadiness 2 "\"petaniMSP\": true" "\"pengepulMSP\": false"

## now approve also for pengepul
approveForMyOrg 2
