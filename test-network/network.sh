...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Create petani Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-petani.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Create pengepul Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-pengepul.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    IMAGE_TAG=${CA_IMAGETAG} docker stack deploy -c $COMPOSE_FILE_CA hlf 2>&1
    sleep 10
    . organizations/fabric-ca/registerEnroll.sh

  while :
    do
      if [ ! -f "organizations/fabric-ca/petani/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generate CCP files for petani and pengepul"
...
function networkUp() {
 ...
 infoln "Delaying 10s"
  sleep 10
  IMAGE_TAG=$IMAGETAG docker stack deploy  ${COMPOSE_FILES} hlf 2>&1
  sleep 5
  docker ps -a
  if [ $? -ne 0 ]; then
...
## call the script to join create the channel and join the peers of petani and pengepul
...
function networkDown() {
  # stop distributor containers also in addition to petani and pengepul, in case we were running sample to add distributor
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/petani/msp organizations/fabric-ca/petani/tls-cert.pem organizations/fabric-ca/petani/ca-cert.pem organizations/fabric-ca/petani/IssuerPublicKey organizations/fabric-ca/petani/IssuerRevocationPublicKey organizations/fabric-ca/petani/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/pengepul/msp organizations/fabric-ca/pengepul/tls-cert.pem organizations/fabric-ca/pengepul/ca-cert.pem organizations/fabric-ca/pengepul/IssuerPublicKey organizations/fabric-ca/pengepul/IssuerRevocationPublicKey organizations/fabric-ca/pengepul/fabric-ca-server.db'
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf addretailer/fabric-ca/retailer/msp addretailer/fabric-ca/distributor/tls-cert.pem addretailer/fabric-ca/retailer/ca-cert.pem addretailer/fabric-ca/retailer/IssuerPublicKey addretailer/fabric-ca/distributor/IssuerRevocationPublicKey addretailer/fabric-ca/distributor/fabric-ca-server.db'
